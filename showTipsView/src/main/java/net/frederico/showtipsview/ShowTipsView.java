...
public class ShowTipsView extends RelativeLayout {
	...
	private void createViews() {
		...
		LinearLayout texts_layout = new LinearLayout(getContext());
		texts_layout.setOrientation(LinearLayout.VERTICAL);
		texts_layout.setVerticalGravity(CENTER_HORIZONTAL);
		/*
		 * Title
		 */
		...
		if (getTitle_color() != 0)
			title.setTextColor(getTitle_color());
		else
		...
		texts_layout.addView(title);

		/*
		 * Description  用于介绍的textView
		 */
//--------------------------------------------------
		TextView content = new TextView(getContext());
		content.setText(getDescription());
		if (getDescription_color() != 0)
			content.setTextColor(getDescription_color());
		else
		...
		btn_close.setBackgroundResource(R.drawable.button_fast_pre);
		...
		RelativeLayout close_relative = new RelativeLayout(getContext());   //包裹着button的relativeLayout
		...
		close_relative.setLayoutParams(params);
		...
		close_relative.addView(btn_close);

		texts_layout.addView(close_relative);
